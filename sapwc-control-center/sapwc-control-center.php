...
add_action( 'admin_notices', function () {
    ...
    if ( SAPWCC_FLAGS_HMAC_SECRET === 'sapwc-flags-hmac-v1-change-me-in-wp-config'
        ...
        echo '<div class="notice notice-warning is-dismissible"><p>'
            ...
            . 'El HMAC secret para flags.json usa el valor por defecto público. '
            . 'Define <code>SAPWCC_FLAGS_HMAC_SECRET</code> en <code>wp-config.php</code> o activa el plugin para generar uno automático.'
            . '</p></div>';
    }
} );
...
add_action( 'wp_ajax_sapwcc_add_site', function () {
    ...
    if ( 0 !== strpos( $url, 'https://' ) ) {
        wp_send_json_error( 'La URL del sitio debe usar HTTPS para proteger el X-SAPWC-Secret en tránsito.' );
    }
    ...
    $result ? wp_send_json_success( 'Sitio añadido.' ) : wp_send_json_error( 'Error al guardar.' );
} );
...
// ─── AJAX: Health check (un sitio o todos) ──────────────────────────────────
...
add_action( 'wp_ajax_sapwcc_save_settings', function () {
    ...
    $token_info  = $saved_token ? 'Token: ' . substr( $saved_token, 0, 6 ) . '...' : 'Token: (vacío)';
    wp_send_json_success( 'Settings guardados. ' . $token_info );
} );

// ─── AJAX: Git push flags.json ──────────────────────────────────────────
...
add_action( 'wp_ajax_sapwcc_git_push_flags', function () {
    ...
    $flags = SAPWCC_Flags::read();
    if ( empty( $flags ) ) {
        wp_send_json_error( 'flags.json vacío o no leído.' );
    }

    $token = SAPWCC_Sites::decrypt( get_option( 'sapwcc_github_token', '' ) );
    if ( empty( $token ) ) {
        wp_send_json_error( 'Token de GitHub no configurado. Guárdalo en Configuración > GitHub Token y pulsa Guardar configuración.' );
    }
    ...
    if ( is_wp_error( $put ) ) {
        wp_send_json_error( 'Error de conexión con GitHub: ' . $put->get_error_message() );
    }
...
// ─── AJAX: Remote action proxy (logs, clear-cache, run-cron, maintenance, update-check) ──
...
// ─── AJAX: Actualizar metadatos de sitio (client_name, email, MRR) ──────
...
add_action( 'wp_ajax_sapwcc_assign_plan', function () {
    ...
    if ( ! in_array( $plan, $valid_plans, true ) ) {
        wp_send_json_error( 'Plan inválido.' );
    }
